...
- [ ]  ログ（最低限）を作る：返信／会議参加／提出物（まずは3つでOK）
- [ ]  Q&Aの更新ルールを決める（誰が、週何回、どこに追記するか）
- [ ]  「よくある迷い」の初期Q&Aを5〜10個だけ先に用意する（自己解決を増やす）
- [ ]  週次会議の型を固定する（Must状況→詰まりTOP3→決める→共有）

### 1月中旬までにやる（Mustを絞るための材料）

- [ ]  困りごとを集計して、Must候補を作る（規律／品質／誇りの3分類で整理）
- [ ]  Must候補ごとに「測り方（ログ）」と「例外が出る場面」を1行で書く
- [ ]  案出しMTGで「暫定Must4つ」を決める（叩き台を確定）
- [ ]  フェーズ3のタスクだし：鈴木担当
